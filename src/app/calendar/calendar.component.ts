  ...
  implements OnInit
{
  ...
  public ngOnInit(): void {
    ...
    this.http.get<any[]>('http://192.168.1.119:8080/api/project_active/').subscribe((data:any) => {
      const events = data.map((ev: any) => ({
        title: ev.Project_client + ' ' + ev.Project_number,
        start: ev.Project_date_created,
        end: ev.Project_end_date,
        color: 'red'
      }))
